...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    // Logged-in users get a stateless server cart (Bearer JWT). Guests keep
    // their cart in localStorage — no cookies anywhere.
    const isAuthed = !!user;
    ...
    // Called right after login: push the guest (localStorage) cart up to the
    // account cart, then drop the local copy and load the server cart.
    const syncAfterLogin = async () => {
        const token = localStorage.getItem("token")
        if (!token) return;

        const storedCart = localStorage.getItem("cartItems")
        const items = storedCart
            ? JSON.parse(storedCart).map((item: CartItem) => ({
        ...
        try {
            if (items.length > 0) {
                await fetch(`${BASEURL}/api/sync-local-cart/`, {
                    method: "POST",
